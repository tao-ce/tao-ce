function(setup)
  {
    env: {
      backend: {
        CONSTRUCT_LANG: setup.defaultLocale,
        CONSTRUCT_BASE_URL: 'https://%s/backoffice/' % setup.publicDomain,
        PORTAL_BE_BASE_URL: 'https://%s/portal-be' % setup.publicDomain,
        DELIVERTENANT_0_CUSTOMER_ID: '1',
        DELIVERTENANT_0_ID: '1',
        DELIVERTENANT_0_LABEL: 'TAO Community Test Runner',
        DELIVERTENANT_0_LTI_1P3_AUDIENCE_URL: 'https://%s' % setup.publicDomain,
        DELIVERTENANT_0_LTI_1P3_CREDENTIALS_0_CLIENT_ID: 'lti-backoffice-deliver',
        DELIVERTENANT_0_LTI_1P3_CREDENTIALS_0_JWKS: '%s/.well-known/jwks.json' % setup.apps['environment-management'].auth_server.http.url,
        DELIVERTENANT_0_LTI_1P3_CREDENTIALS_COUNT: '1',
        DELIVERTENANT_0_LTI_1P3_LAUNCH_URL: 'https://%s/deliver/api/v1/auth/launch-lti-1p3' % setup.publicDomain,
        DELIVERTENANT_0_LTI_1P3_OIDC_LOGIN_INITIATION_URL: 'https://%s/auth-server/lti1p3/oidc/initiation' % setup.publicDomain,
        DELIVERTENANT_0_OAUTH_CREDENTIALS_0_KEY: 'lti-backoffice-deliver',
        DELIVERTENANT_0_OAUTH_CREDENTIALS_0_ROLES_0: 'Learner',
        DELIVERTENANT_0_OAUTH_CREDENTIALS_0_ROLES_1: 'ContentDeveloper',
        DELIVERTENANT_0_OAUTH_CREDENTIALS_0_ROLES_COUNT: '2',
        DELIVERTENANT_0_OAUTH_CREDENTIALS_0_SECRET: setup.lib.hash('lti-backoffice-deliver'),
        DELIVERTENANT_0_OAUTH_CREDENTIALS_COUNT: '1',
        DELIVERTENANT_0_ROOT_URL: '%s/api/v1/' % setup.apps.deliver.backend.http.url,
        DELIVERTENANT_0_TOKEN_URL: '%s/v1/oauth2/tokens' % setup.apps['environment-management'].auth_server.http.url,
        DELIVERTENANT_0_DYNAMIC_QUERY_API_URL: setup.apps.dynamic_query.api.http.url + '/api/v1',
        SCORING_SERVICE_URL: 'https://%(publicDomain)s/ss-be/api/v1/events' % setup,
        DELIVERTENANT_COUNT: '1',
        ELASTICSEARCH_HOSTS: setup.dependencies.es.address.baseUrl,
        FEATURE_FLAG_ENABLE_DATA_STORE_STORAGE: 'true',
        FEATURE_FLAG_FLA: 'true',
        // FEATURE_FLAG_QUICK_WINS_ENABLED: "true",
        // FEATURE_FLAG_STATISTIC_METADATA_IMPORT: "true",
        // enable new Test preview
        FEATURE_FLAG_TAO_ADVANCE_ONLY: 'true',
        FEATURE_FLAG_TAO_CG_ONLY: 'false',
        FEATURE_FLAG_TAO_ADVANCE_ONLY_CORE: 'true',
        // FEATURE_FLAG_TRANSLATION_ENABLED: "true",
        FEATURE_FLAG_UNIQUE_NUMERIC_QTI_IDENTIFIER: 'true',
        GOOGLE_CLOUD_DELIVERY_EXECUTION_PUB_SUB_TOPIC_ID: 'ss-delivery-executions-topic',
        GOOGLE_CLOUD_DELIVERY_PUB_SUB_TOPIC_ID: 'ss-deliveries-topic',
        GOOGLE_CLOUD_PROJECT: setup.env.GOOGLE_CLOUD_PROJECT,
        LTI_AUTHORIZATION_SERVER_FACTORY_ENCRYPTION_KEY: setup.lib.hash('construct/env/LTI_AUTHORIZATION_SERVER_FACTORY_ENCRYPTION_KEY'),
        SCORING_CLIENT_ID: 'ss-solar-client-id-for-tao',
        SCORING_CLIENT_SECRET: setup.lib.hash('ss-solar-client-id-for-tao'),
        SCORING_ENABLE: 'true',
        SCORING_SCOPE: 'event:post',
        IS_QTI_PACKAGE_STORED_EXTERNALLY_ENABLED: 'true',
        PUBLIC_STORAGE_URL: '%s/construct/data' % setup.dirs.varlib,
        SCORING_TOKEN_URL: '%s/v1/oauth2/tokens' % setup.apps['environment-management'].auth_server.http.url,
        XDEBUG_MODE: 'off',

        LISTEN_PORT: setup.apps.construct.backend.http.port,
      },
    },
    files: {
      'construct-seed.json': std.manifestJson((import './files/construct-seed.libsonnet')(setup)),
    },

  }
